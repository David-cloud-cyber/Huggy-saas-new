...
async function run() {
  ...
  assert.ok(create.json().subdomain.domain.endsWith('.huggy.app'), 'project gets SaaS subdomain');
  ...
  const crossUser = await app.inject({ method: 'GET', url: `/api/projects/${projectId}`, headers: { authorization: 'Bearer test-user-2' } });
  assert.equal(crossUser.statusCode, 404, 'cross-user project access is blocked');

  const config = await app.inject({ method: 'GET', url: '/api/admin/config', headers: auth });
  assert.equal(config.statusCode, 200, 'admin config health endpoint is available');
  assert.equal(typeof config.json().integrations.openRouter, 'boolean', 'config endpoint returns booleans only');
  ...
  const forbiddenCustom = await app.inject({
    method: 'POST',
    url: '/api/ai/route',
    headers: auth,
    payload: { actionKind: 'chat_simple', mode: 'custom', customModelId: 'openai/not-real' }
  });
  assert.equal(forbiddenCustom.statusCode, 404, 'arbitrary frontend model id is blocked');
  ...
  assert.ok(domain.json().dns.length > 0, 'custom domain returns DNS instructions');
  const domainId = domain.json().domain.id;

  const verifyDomain = await app.inject({
    method: 'POST',
    url: `/api/projects/${projectId}/domains/${domainId}/verify`,
    headers: auth
  });
  assert.equal(verifyDomain.statusCode, 200, 'domain verification updates status');

  const primaryDomain = await app.inject({
    method: 'PATCH',
    url: `/api/projects/${projectId}/domains/${domainId}/primary`,
    headers: auth
  });
  assert.equal(primaryDomain.statusCode, 200, 'verified domain can become primary');
  ...
  const reservedSlug = await app.inject({
    ...
    url: `/api/projects/${projectId}/domains`,
    headers: auth,
    payload: { domain: 'admin', type: 'subdomain' }
  });
  assert.equal(reservedSlug.statusCode, 409, 'reserved SaaS subdomain slug is blocked');
  ...
  assert.ok(deploy.json().deployment_steps.includes('ready'), 'deployment exposes streamed status contract');

  const rollback = await app.inject({
    method: 'POST',
    url: `/api/projects/${projectId}/deployments/${deploy.json().deployment.id}/rollback`,
    headers: auth
  });
  assert.equal(rollback.statusCode, 200, 'rollback endpoint accepts previous deployment');

  const topup = await app.inject({
    method: 'POST',
    url: '/api/billing/checkout/topup',
    headers: auth,
    payload: { credits: 100 }
  });
  assert.equal(topup.statusCode, 200, 'top-up checkout has mock contract in test');
