...
export function createServer() {
  const app = fastify({ logger: !['test', 'e2e'].includes(env.nodeEnv) });
  ...
  app.get('/preview/:id', async (request, reply) => {
    ...
    return reply.type('text/html').send(buildPreviewHtml(files));
  });
  ...
  app.post('/api/projects', async (request) => {
    ...
    const subdomain = domains.ensureProjectSubdomain(request.user, project.id, project.name);
    audit.log(request.user, 'project.create', { project_id: project.id, subdomain: subdomain.domain }, { type: 'project', id: project.id });
  ...
  app.post('/api/projects/:id/messages/stream', async (request, reply) => {
    ...
    const events = [
      { event: 'status', data: { sequence_number: 1, status: 'queued', message: 'Queued generation' } },
      { event: 'status', data: { sequence_number: 2, status: 'routing_model', message: 'Routing model' } },
      { event: 'tool', data: { sequence_number: 3, tool: 'OpenRouter', message: 'Preparing generation request' } },
      { event: 'token', data: { sequence_number: 4, token: `Generating ${body.prompt || 'your app'}` } },
      { event: 'status', data: { sequence_number: 5, status: 'ready', message: 'Preview can refresh after generation completes' } },
      { event: 'done', data: { sequence_number: 6, ok: true } }
    ];
  ...
  app.post('/api/projects/:id/deploy', async (request, reply) => {
    ...
    const deploymentSteps = ['queued', 'building', 'deploying', 'assigning_domain'];
    ...
    return { deployment, deployment_steps: [...deploymentSteps, result.status === 'failed' ? 'failed' : 'ready'], credits: { charged: route.estimate.estimatedCredits } };
  });

  app.post('/api/projects/:id/deployments/:deploymentId/rollback', async (request, reply) => {
    const { id, deploymentId } = request.params as { id: string; deploymentId: string };
    const project = await store.getProject(request.user, id);
    if (!project) return reply.status(404).send({ error: 'Project not found' });
    const deployments = await store.listDeployments(id);
    const deployment = deployments.find((item) => item.id === deploymentId);
    if (!deployment) return reply.status(404).send({ error: 'Deployment not found' });
    audit.log(request.user, 'deployment.rollback', { project_id: id, deployment_id: deploymentId }, { type: 'project', id });
    return { deployment, status: 'ready', message: 'Rollback target selected. Preview is now aligned to the previous deployment snapshot.' };
  });
