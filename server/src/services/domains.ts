...
export class DomainService {
  ...
  ensureProjectSubdomain(user: UserContext, projectId: string, projectName: string) {
    const existing = domains.find((domain) => (
      domain.organization_id === user.id &&
    ...
    const slug = uniqueSlug(projectName, user.id);
    return this.add(user, projectId, { domain: slug, type: 'subdomain', isPrimary: true }).domain;
  }
...
function uniqueSlug(value: string, ownerId: string) {
  const base = value
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, '-')
    .replace(/^-|-$/g, '')
    .slice(0, 32) || 'project';
  const suffix = ownerId.replace(/[^a-z0-9]/gi, '').slice(0, 6).toLowerCase() || 'user';
  let slug = `${base}-${suffix}`;
  let counter = 2;
  while (domains.some((domain) => domain.domain === `${slug}.${env.huggyPublicDomain}` && domain.status !== 'removed')) {
