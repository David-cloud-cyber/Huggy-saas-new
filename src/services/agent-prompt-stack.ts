...
import {
  HUGGY_COMMUNICATION_VALIDATION_RULES,
  HUGGY_INTERLEAVED_COMMUNICATION_PROTOCOL,
} from '../lib/prompts/communication.ts';
import {
  HUGGY_AUTO_PROVISIONED_INFRASTRUCTURE_CONTRACT,
  HUGGY_INFRASTRUCTURE_PROVISIONING_STATES,
} from '../lib/prompts/infrastructure.ts';
import {
  HUGGY_MESSAGE_PART_RENDERING_RULES,
  HUGGY_STRUCTURED_MESSAGE_STREAMING_CONTRACT,
} from '../lib/prompts/message-streaming.ts';
import {
...
const HUGGY_CLOUD_POLICY = [
  ...
  'Include supabase/schema.sql when persistent data is needed, but generated previews must be honest: if Huggy Cloud is only planned and not active yet, show an unavailable-backend state and never claim real persistence is live.',
  ...
  'If Huggy Cloud runtime config is not available in preview, show a safe unavailable/auth-unavailable state instead of crashing. Auth UI can be shown, but real sign-in must be clearly unavailable until Huggy Cloud is active.',
...
const HUGGY_AI_CONNECTOR_POLICY = [
  ...
  'Separate two concepts: Huggy editor progress is the compact rich-parts stream, while generated-app AI streaming is product code inside the user app.',
...
const HUGGY_GENERATION_PRODUCT_POLICY = [
  ...
  'Select exactly one Huggy runtime profile. Use tanstack-fullstack for real applications with backend data, authentication, storage, realtime, payments, server functions, SSR, SEO, or publication. Use vite-static only for genuinely static apps without a backend.',
  'The tanstack-fullstack profile uses TanStack Start, TanStack Router with file-based routes, TanStack Query for server state, React, strict TypeScript, Tailwind CSS, Zod, Supabase/Huggy Cloud, and Cloudflare Workers. Vite remains the build tool.',
  'The vite-static profile uses React, strict TypeScript, Vite, Tailwind CSS, and only dependencies that are required by the product.',
  'Generation stack v2 is mandatory for every generated app: use the selected Huggy runtime profile and preserve a complete, versioned, buildable project contract.',
  'Generation stack v2 remains the compatibility contract for legacy Vite projects: React 18, Vite, strict TypeScript, Tailwind CSS, and complete configuration files.',
  'Legacy Vite templates use React 18 and include tailwind.config.ts, src/main.tsx, and the existing smoke-test contract while projects migrate to the TanStack profile.',
  'For new apps, return a complete profile-specific project including package.json, the correct Vite/TanStack configuration, tsconfig.json, index.html or route entry, src/, README.md, tests, wrangler.jsonc for Cloudflare Workers profiles, and huggy/app-manifest.json.',
  'The generated app manifest must declare profile, framework, runtime, backend, build command, output directory, routes, public/server environment variables, capabilities, and acceptance criteria. Never claim TanStack Start when the generated files do not actually contain TanStack Start.',
  'For tanstack-fullstack, use @tanstack/react-start, @tanstack/react-router, @tanstack/react-query, @cloudflare/vite-plugin, and wrangler with versions pinned by the Huggy template lockfile. Do not use Next.js patterns, legacy TanStack release-candidate APIs, or invented server directives.',
  'For vite-static, index.html must be a Vite shell with <div id="root"></div> and a module script for /src/main.tsx. Do not put the whole app in index.html.',
  'src/main.tsx must import React, ReactDOM from react-dom/client, App from ./App, and ./index.css when the selected profile is vite-static.',
  'Vite/TanStack configuration must use strict TypeScript. TanStack routes must be file-based, parameters and search params must be typed, and server/client boundaries must remain explicit.',
  ...
  'If the app needs persistent backend data or auth, include @supabase/supabase-js ^2.106.0, src/lib/huggyCloud.ts, src/lib/appData.ts, src/lib/validation.ts, supabase/schema.sql, versioned migrations, RLS, policies, indexes, timestamps, and owner/user or workspace scoping.',
  'Legacy Vite backend compatibility may include src/lib/supabaseClient.ts with VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY only; new projects should prefer src/lib/huggyCloud.ts.',
...
const HUGGY_ZERO_BUG_GENERATION_POLICY = [
  ...
  'For every new app, resolve exactly one runtime profile: tanstack-fullstack for backend/fullstack/SSR applications, or vite-static for genuinely static applications. Use HTML-only only when the user explicitly requests a simple static HTML page.',
  'Every new app must include complete profile-specific files: package.json, configuration, source entry/routes, tests, README.md, and huggy/app-manifest.json. tanstack-fullstack additionally includes src/server.ts, src/routes/, src/lib/query-client.ts, and wrangler.jsonc.',
...
export function buildIntentRouterSystemPrompt() {
  return joinSections([
    ...
    [
      ...
      'Schema: {"intent":string,"intent_category":string,"confidence":number,"auto_plan_required":boolean,"selected_model_policy":"economy|balanced|premium","reason":string,"user_visible_reason":string,"normalized_prompt":string,"required_capabilities":string[],"objective":{"goal":string,"scope":{"included":string[],"excluded":string[]},"constraints":string[],"assumptions":string[],"acceptanceCriteria":string[],"risk":"low|medium|high|critical"},"clarification":{"question":string,"choices":string[],"recommendation":string}}.',
      'The objective and required_capabilities fields are mandatory even for conversation or clarification. Never omit them and never use null placeholders.',
