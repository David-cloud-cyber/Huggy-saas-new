...
function buildReactVitePreviewHtml(
  ...
  const html = [
    ...
    '            if (prop === "default") return function() { return null; };',
...
function buildDeterministicFallbackGeneratedOutput(projectName: string, promptOrDescription = '') {
  ...
  const appContent = isPomodoro
    ...
    ? [
        ...
        '    <main className="min-h-screen bg-slate-50 px-4 py-10 text-slate-950 sm:px-6 lg:px-8">',
        '      <section className="mx-auto max-w-5xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">',
        '        <span className="text-xs font-bold uppercase tracking-[0.22em] text-blue-600">Interactive todo workspace</span>',
        `        <h1 className="mt-4 text-4xl font-bold tracking-tight sm:text-6xl">{${safeName}}</h1>`,
        `        <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600">{${safePrompt}}</p>`,
      '      </section>',
      '',
        '      <section className="mx-auto mt-6 grid max-w-5xl gap-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8" aria-label="Todo app">',
        '        <div className="grid gap-3 sm:grid-cols-3">',
        '          <div className="rounded-2xl bg-slate-50 p-4"><strong className="block text-3xl">{todos.length}</strong><span className="text-sm text-slate-500">Total</span></div>',
        '          <div className="rounded-2xl bg-slate-50 p-4"><strong className="block text-3xl">{activeCount}</strong><span className="text-sm text-slate-500">Active</span></div>',
        '          <div className="rounded-2xl bg-blue-50 p-4"><strong className="block text-3xl text-blue-700">{completedCount}</strong><span className="text-sm text-blue-700">Completed</span></div>',
      '        </div>',
      '',
        '        <form className="grid gap-3 sm:grid-cols-[1fr_auto]" onSubmit={addTodo}>',
      '          <input',
        '            className="min-h-12 rounded-full border border-slate-200 bg-white px-4 text-slate-950 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"',
      '            value={draft}',
      '            onChange={event => setDraft(event.target.value)}',
      '            placeholder="Add a task..."',
      '            aria-label="New task"',
      '          />',
        '          <button className="min-h-12 rounded-full bg-slate-950 px-5 font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50" type="submit" disabled={!draft.trim()}>Add</button>',
      '        </form>',
      '',
        '        <div className="flex flex-wrap gap-2" aria-label="Todo filters">',
      "          {(['all', 'active', 'completed'] as TodoFilter[]).map(option => (",
      '            <button',
      '              key={option}',
        "              className={filter === option ? 'rounded-full bg-blue-600 px-4 py-2 text-sm font-semibold text-white' : 'rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-blue-300 hover:text-blue-700'}",
      ...
      '              type="button"',
        ...
        '        </div>',
        '',
        '        {feedback ? <p className="rounded-2xl bg-blue-50 px-4 py-3 text-sm font-medium text-blue-800" role="status">{feedback}</p> : null}',
      '',
        '        <div className="grid gap-3">',
      '          {visibleTodos.length ? visibleTodos.map(todo => (',
        '            <article className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between" key={todo.id}>',
        '              <label className="flex items-center gap-3">',
        '                <input className="size-5 accent-blue-600" type="checkbox" checked={todo.completed} onChange={() => toggleTodo(todo.id)} />',
        '                <span className={todo.completed ? "text-slate-400 line-through" : "text-slate-900"}>{todo.title}</span>',
      '              </label>',
        '              <button className="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-red-200 hover:bg-red-50 hover:text-red-700" type="button" onClick={() => deleteTodo(todo.id)} aria-label={`Delete ${todo.title}`}>Delete</button>',
      '            </article>',
      '          )) : (',
        '            <div className="grid gap-1 rounded-2xl border border-dashed border-slate-300 p-6 text-center text-slate-500">',
    ...
    : [
        ...
        '    <main className="min-h-screen bg-slate-50 px-4 py-10 text-slate-950 sm:px-6 lg:px-8">',
        '      <section className="mx-auto max-w-5xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">',
        '        <span className="text-xs font-bold uppercase tracking-[0.22em] text-blue-600">Generated application</span>',
        `        <h1 className="mt-4 text-4xl font-bold tracking-tight sm:text-6xl">{${safeName}}</h1>`,
        `        <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600">{${safePrompt}}</p>`,
        '        <button className="mt-8 rounded-full bg-slate-950 px-5 py-3 font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-200" type="button" onClick={() => setStatus("Interaction confirmed")}>Try primary action</button>',
      '      </section>',
        '      <section className="mx-auto mt-6 grid max-w-5xl gap-3 sm:grid-cols-3">',
        '        {features.map(feature => <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" key={feature}><strong className="block text-lg">{feature}</strong><span className="mt-2 block text-sm text-slate-500">{status}</span></article>)}',
        '      </section>',
        '    </main>',
        '  );',
        '}',
